...
#define _ompnode_debug_ false
#define _ompnode_output_ false


fks_ompNode::~fks_ompNode()
{
    //cout<<"level = "<<level<<", lnid = "<<lnid<<endl;
    if(skeletons != NULL) {
        delete skeletons;
        skeletons = NULL;
        //cout<<"delete skeletons done"<<endl;
    }
    if(leftNode != NULL) {
        delete leftNode;
        leftNode = NULL;
        //cout<<"delete left child done"<<endl;
    }
    if(rightNode != NULL) {
        delete rightNode;
        rightNode = NULL;
        //cout<<"delete right child done"<<endl;
    }
 }
                         ...
                         int mppn, int maxlevel)
{
    ...
    if (in_parent!=NULL)  {
      ...
      // this will make the levels of the shared memory tree correct from the 
      // beginning, I think -- Bill
      //level = morton_offset;
      // we don't want this, because the shared level needs to stay small
	...
	if( level == maxlevel || numof_points <= mppn )
    {
        ...
        leaf_point_gids.resize(numof_points);
        leaf_point_local_id.resize(numof_points);
        for(int i = 0; i < numof_points; i++) {
            leaf_point_gids[i] = inData->gids[(*active_set)[i]];
            leaf_point_local_id[i] = (*active_set)[i];
        }
        delete active_set; active_set = NULL;
        
        // set the number of points owned
        num_points_owned = numof_points;

        if(_ompnode_output_) {
            cout<<"\t\t base case done"<<endl;
        }
    ...
    proj.resize(dim);
    // here inData would not change, so I need to copy the
    // corresponding point to get the correct projection direction
    double *points = new double [numof_points*dim];
    #pragma omp parallel for
    for(int i = 0; i < numof_points; i++) {
        int idx = (*active_set)[i];
        memcpy( points+i*dim, &(inData->X[idx*dim]), sizeof(double)*dim );
    }
    mtreeProjection(points, numof_points, dim, &(proj[0]), &(px[0]));
    delete [] points;

    if(_ompnode_output_) {
        cout<<"\t\t generate projection and projected values done"<<endl;
    }
  ...
  num_points_owned = leftNode->num_points_owned + rightNode->num_points_owned;
