...
*/
	...
	double r = knn::lsh::rSelect(&(data.X[0]), &(data.gids[0]), &(query.X[0]), &(query.gids[0]), 
				numof_points*nproc, numof_points*nproc, numof_points, numof_points, dim,
				20, 0.5, MPI_COMM_WORLD);
...
*/
	...
	queryR_ApproxK( &query, numof_points*nproc, 5.0, &root,
             r, 20, 3.0, 200, neighbors, outQueryIDs);

/*
	queryR(&query, &root, o.range, neighbors);
	if(o.dbg_out) {
		if(rank == 0) remove("treeQueryResult.check");
		MPI_Barrier(MPI_COMM_WORLD);
		ofstream output_result;
		if(rank == 0) {
			output_result.open("treeQueryResult.check", ios::app|ios::out);
