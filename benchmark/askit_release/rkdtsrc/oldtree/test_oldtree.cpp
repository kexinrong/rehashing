...
int main(int argc, char **argv) {
	...
	int traverse_type;
	cmd.addICmdOption("-tt", &traverse_type, 0, "specify query traverse type, (*0*): new rkdt, (1): old rkdt");
	...
	if(a2a_verbose == 1) {		// use all to all special case
		oldtree::knnTreeSearch_RandomRotation_a2a(&refData, k, 
								numiters, params, tree_rotation_type, tree_coord_type,
								queryIDs_rkdt, kNN_rkdt);
	}
	else {
		oldtree::knnTreeSearch_RandomRotation(&refData, &queryData, k, 
									numiters, params, tree_rotation_type, tree_coord_type,
									queryIDs_rkdt, kNN_rkdt);
	}
