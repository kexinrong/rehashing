...
int main(int argc, char **argv) {
   ...
   switch(gen) {
     ...
     case 4:
       ...
       generateNormalEmbedding(nPts, dim, 10, ref, MPI_COMM_WORLD);
       break;
   ...
   if(check) {
     get_sample_info(ref, query, ids, queryids, nPts, nPts, dim, k,
   ...
   int tmpK = K;
   if(autotune) {
         knn::lsh::rKSelect(ref, ids, query, queryids, nglobal, nglobal, nPts, nPts,
   ...
   if(rank == 0) cout << "r: " << r << endl;
      ...
      knn::lsh::distPartitionedKQuery( ref, ids, query, queryids, nglobal, nglobal, nPts,
                   nPts, dim, r, k, K, L, bf, errorTarget, lsh_results, outIDs, MPI_COMM_WORLD  );
