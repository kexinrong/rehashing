...
int main(int argc, char **argv) {
   ...
   switch(gen) {
     ...
     case 4:
       if(rank == 0) cout << "Distribution: "<<indim<<"-d unit gaussian embedded in " << dim << " dimensions"  << endl;
       generateNormalEmbedding(nPts, dim, indim, ref, MPI_COMM_WORLD);
       break;
   ...
   if(check) {
     if(rank == 0) cout << "Sampling points for verification..." << endl;
   ...
   if(rank == 0) cout << "r: " << r << endl;
      ...
      if (rank == 0) cout << "Overall time: " << querytime + rtime << endl;
...
void getArgs(int argc, char **argv){
        ...
        cmd.addICmdOption("-id", &indim, 1, "Intrinsic dimension of the space (default = 1).");
        ...
        cmd.addICmdOption("-gen", &gen, 1, "Data generator (1). 0=uniform, 1=hypersphere, 2=unit guassian, 3=mix of gaussians, 4=embedded unit gaussian");
