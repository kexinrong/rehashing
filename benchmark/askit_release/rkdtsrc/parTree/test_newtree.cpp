...
int main(int argc, char **argv) {
	...
	if(rank == 0) cout<<"new tree start ..."<<endl;
	...
	int max_tree_level;
	cmd.addICmdOption("-mtl", &max_tree_level, 30, "maximum tree depth (*30*)");
	...
	int eval_verbose;
	cmd.addICmdOption("-eval", &eval_verbose, 0, "verify results (*0*). 0: do not verify, 1: check uisng logm sampled points");
	...
	int tree_coord_type;
	cmd.addICmdOption("-fc", &tree_coord_type, 0, "kd tree coordinates choose (*0*). 0: choose randomly, 1: choose coord with max variance");
	...
	switch(gen) {
		...
		case 5:
		{
			...
			//for(int i = 0; i < total_numof_input_points*dim; i++) query[i] = dummy_points[i] / 1.0;
	...
	if(a2a_verbose == 1) {		// use all to all special case
		rkdt_a2a(refData, k, numiters, max_points_per_node, max_tree_level, 
				 tree_rotation_type, tree_coord_type, eval_verbose,
				 queryIDs_rkdt, kNN_rkdt, MPI_COMM_WORLD);
	}
	else {
		rkdt(refData, queryData, k, numiters, max_points_per_node, max_tree_level, 
				tree_rotation_type, tree_coord_type, eval_verbose,
				 queryIDs_rkdt, kNN_rkdt, MPI_COMM_WORLD);
	}
	...
	if(flops_verbose == 1) {
		...
		// overall repartition time  
		double mpi_collective_t = MPI_Collective_Const_T_ + MPI_Collective_Query_T_;
		MPI_Reduce(&mpi_collective_t, &max_t, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
		MPI_Reduce(&mpi_collective_t, &min_t, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
		MPI_Reduce(&mpi_collective_t, &avg_t, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
		if(rank == 0) {
			cout<<" + MPI Collective (Overall): "
				<<"\tmin: "<<min_t
				<<"\tmax: "<<max_t
				<<"\tavg: "<<avg_t / (double)size
				<<endl;
