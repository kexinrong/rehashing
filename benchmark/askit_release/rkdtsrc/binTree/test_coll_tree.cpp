...
int main(int argc, char **argv) {
	...
	int traverse_type;
	cmd.addICmdOption("-tt", &traverse_type, 0, "specify query traverse type, (*0*): new rkdt, (1): old rkdt, (2): rsmt");
	...
	root->options.splitter = "rkdt";
	root->options.pruning_verbose = 1;
	root->options.flag_r = tree_rotation_type;
	...
	gatherTree(root, ArrTree);
	MPI_Barrier(MPI_COMM_WORLD);
	
	if(rank == 0) {
		cout<<"ArrTree.size() = "<<ArrTree.size()<<endl;
		for(int i = 0; i < ArrTree.size(); i++)
			cout<<ArrTree[i].first<<" "<<ArrTree[i].second<<" "<<endl;
	}


	int depth = int( ceil( log((double)(ArrTree.size()+1)) / log(2.0) ) );
	pstNode stroot = new stNode();
	plantTree(ArrTree, depth, NULL, stroot);
	
	if(rank == 0) printTree(stroot);

	int * point_to_visual_leaf_membership = new int [numof_query_points];
	vector<int> member_ids(numof_query_points);
	for(int i = 0; i < numof_query_points; i++)
		member_ids[i] = i;
	int tmp_nc = (int)pow(2.0, (double)depth);
	int * count = new int [tmp_nc];
	visitTree(&(queryData.X[0]), numof_query_points, queryData.dim,
			  member_ids, stroot, depth, 
	...
	if(tree_dbg_out) {
		for(int t = 0; t < size; t++) {
			if(t == rank) {
				cout<<rank<<" : mem: ";
				for(int i = 0; i < numof_query_points; i++)
					cout<<point_to_visual_leaf_membership[i]<<" ";
				cout<<endl;
				cout<<rank<<" : count: ";
	...
	long *newgids;
	long newN;
	repartitionQueryData(&(queryData.X[0]), &(queryData.gids[0]), queryData.numof_points, queryData.dim,
						 root, &newX, &newgids, newN);

	
	if(tree_dbg_out) {
		if(rank == 0) cout<<"after repartition: "<<endl;
		MPI_Barrier(MPI_COMM_WORLD);
		for(int t = 0; t < size; t++) {
			if(t == rank) {
