	...
	switch(gen) {
		...
		case 5:
		{
			...
			if(a2a_verbose) {
                ...
                int dummy_n = numof_ref_points;
				//knn::parallelIO(strInputFile, total_numof_input_points, dim, dummy_points, MPI_COMM_WORLD);
				//knn::mpi_binread(strInputFile, total_numof_input_points, dim, dummy_n, dummy_points, MPI_COMM_WORLD);
                total_numof_input_points = dummy_n;
