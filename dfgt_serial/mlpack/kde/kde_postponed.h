...
class KdePostponed {
  ...
  public:
    ...
    void ApplyContribution(
      ...
      double density_incoming =
        global.kernel().Evaluate(
          metric, query_point, reference_point, reference_weight);
    ...
    template<typename GlobalType>
    void SetZero(const GlobalType &global) {
      this->SetZero();
    }
