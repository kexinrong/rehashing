...
void DualtreeDfs<ProblemType>::Compute(
  ...
  if(do_initializations) {
    ...
    // Then the query tree.
    PreProcess(
      problem_->global(), query_table_,
      query_table_->get_tree(), query_results, 0);
  }
...
void DualtreeDfs<ProblemType>::PreProcess(
  const typename ProblemType::GlobalType &global,
  ...
  qnode_stat.Seed(global, initial_pruned);

  if(! qnode->is_leaf()) {
    PreProcess(
      global, query_table_in, qnode->left(), query_results, initial_pruned);
    PreProcess(
      global, query_table_in, qnode->right(), query_results, initial_pruned);
  }
  else {
