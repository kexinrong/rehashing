...
class Table {
  ...
  public:
    ...
    void Init(
      ...
      int num_weights_per_point_in = 1) {
      rank_ = rank_in;
      if(num_dimensions_in > 0 && num_points_in > 0) {
        data_.zeros(num_dimensions_in, num_points_in);
        weights_.set_size(num_weights_per_point_in, num_points_in);
        weights_.fill(1.0);
        if(core::table::global_m_file_) {
    ...
    void Save(
      ...
      for(unsigned int j = 0; j < data_.n_cols; j++) {
        ...
        if(woutput != NULL) {
          for(int i = 0; i < static_cast<int>(weights_.n_rows); i++) {
            fprintf(woutput, "%g", point_weight[i]);
            if(i < static_cast<int>(weights_.n_rows) - 1) {
              fprintf(woutput, ",");
            }
          }
          fprintf(woutput, "\n");
     ...
     */
    void get(
      int point_id, arma::vec *point_out, arma::vec *point_weight_out) const {
     ...
     */
    void get(
      int point_id, arma::vec *point_out, arma::vec *point_weight_out) {
      direct_get_(point_id, point_out, point_weight_out);
    }
  ...
  private:
    ...
    void direct_get_(
      int point_id, arma::vec *entry, arma::vec *point_weight) const {
      if(this->IsIndexed() == false) {
        core::table::MakeColumnVector(data_, point_id, entry);
        core::table::MakeColumnVector(weights_, point_id, point_weight);
      }
      else {
        ...
        core::table::MakeColumnVector(
          weights_, located_position, point_weight);
      }
    }

    void direct_get_(
      int point_id, arma::vec *entry, arma::vec *point_weight) {
      if(this->IsIndexed() == false) {
        core::table::MakeColumnVector(data_, point_id, entry);
        core::table::MakeColumnVector(weights_, point_id, point_weight);
      }
    ...
    void iterator_get_(
      int reordered_position, arma::vec *entry, arma::vec *weight) const {
      int located_reordered_position =
        this->locate_reordered_position_(reordered_position);
      core::table::MakeColumnVector(
        data_, located_reordered_position, entry);
      core::table::MakeColumnVector(
        weights_, located_reordered_position, weight);
    }
